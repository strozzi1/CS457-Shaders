...
void main(){
    ...
    int numLayers = 1 << uLevel;  //number of levels of triangle subdivisions
    
    float dt = 1. / float( numLayers );         //from spheresubd
    float t_top = 1.;

    for(int it = 0; it<numLayers; it++){        //from spheresubd code
...
void ProduceVertex(float s, float t){
    ...
    vec3 v = V0 + s*V01 + t*V02;            //from spheresubd
    if(!uModelCoords){
